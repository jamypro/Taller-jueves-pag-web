...
(function () {
  ...
  if (temaGuardado === TEMA_OSCURO) document.body.classList.add(CLASE_OSCURO);
})();
...
function inicializarTema() {
  ...
  document.addEventListener("keydown", function (evento) {
    if (evento.altKey && evento.key === "t") alternarTema();
  });
}

function alternarTema() {
  const estaOscuro = document.body.classList.toggle(CLASE_OSCURO);
  if (estaOscuro) localStorage.setItem(CLAVE_TEMA, TEMA_OSCURO);
  else localStorage.setItem(CLAVE_TEMA, TEMA_CLARO);
  actualizarIcono();
  if (typeof window.ajustarOla === "function") window.ajustarOla();
  const botonTema = document.getElementById("botonTema");
  if (botonTema) {
...
window.obtenerTemaActual = function () {
  return document.body.classList.contains(CLASE_OSCURO)
    ? TEMA_OSCURO
    : TEMA_CLARO;
};
...
function respetarPreferenciaSistema() {
  ...
  window
    ...
    .addEventListener("change", function (evento) {
      ...
      if (evento.matches) document.body.classList.add(CLASE_OSCURO);
      else document.body.classList.remove(CLASE_OSCURO);
      actualizarIcono();
    });
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const temaActual = document.body.classList.contains(CLASE_OSCURO)
    ? "🌙 Oscuro"
    : "☀️ Claro";
  console.log(`🎨 tema.js cargado. Tema actual: ${temaActual}`);
  console.log(
    "💾 Preferencia guardada en localStorage con la clave:",
    CLAVE_TEMA,
  );
});
